#!/usr/bin/env python3
"""Analysis of milestone 4 log 5 - check convergence and final tuning quality"""
import pandas as pd
import numpy as np
...
# Check for debug columns that might show PID values
debug_cols = [c for c in df.columns if 'debug' in c.lower()]
print(f"Debug columns: {debug_cols}")

# Try to extract PID values from debug if available
if 'debug[0]' in df.columns:
    print(f"\nDebug[0] range: {df['debug[0]'].min()} to {df['debug[0]'].max()}")
if 'debug[1]' in df.columns:
    print(f"Debug[1] range: {df['debug[1]'].min()} to {df['debug[1]'].max()}")
if 'debug[2]' in df.columns:
    print(f"Debug[2] range: {df['debug[2]'].min()} to {df['debug[2]'].max()}")
if 'debug[3]' in df.columns:
    print(f"Debug[3] range: {df['debug[3]'].min()} to {df['debug[3]'].max()}")
...
if len(results_df) > 0:
    ...
    # Show maneuver-by-maneuver progression
    print()
    print('='*100)
    print('MANEUVER-BY-MANEUVER PROGRESSION')
    print('='*100)
    print(f"{'#':>3} {'Time':>8} {'Lag(ms)':>10} {'Error':>10} {'Overshoot':>12} {'Ratio':>8}")
    print('-'*60)
    for i, (idx, row) in enumerate(results_df.iterrows()):
        print(f"{i+1:3d} {row['time']:8.1f} {row['lag_ms']:10.1f} {row['tracking_error']:10.1f} "
              f"{row['overshoot']:11.1f}% {row['peak_ratio']:8.2f}")
    ...
    plt.suptitle('Autotune Convergence Analysis - Log 5 (Newton\'s Method)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('m4_log5_convergence.png', dpi=150)
    print("\nSaved convergence plot to m4_log5_convergence.png")
    ...
    plt.suptitle('Early vs Late Maneuver Comparison - Log 5', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('m4_log5_comparison.png', dpi=150)
    print("Saved comparison plot to m4_log5_comparison.png")
    ...
    if len(good_results) > 0:
        ...
        first_good_idx = results_df[results_df['time'] < first_good_time].shape[0]
        ...
        print(f"Iterations before good: {first_good_idx} maneuvers")
    else:
